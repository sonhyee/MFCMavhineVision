...
CInspectionHandler::CInspectionHandler()
	...
	m_nTotalModelCnt(0), m_nFinModelCnt(0), m_hWnd(NULL), m_strResText("")
{}
...
void CInspectionHandler::ProcessingCamQueue()
{
	while (m_bRunCamThread)
	{
		...
		case GRAB:
		{
			...
			tmp.append("00");
			tmp += to_string(GetModelCount());
			tmp += ".jpg";
			const char* imgName = tmp.c_str();

			if (!m_Patterns[m_nGrabCnt].SetPtnImg(imgName))
			{
				while (!m_CamQueue.empty())	m_CamQueue.pop();
	...
	while (m_bRunAlgThread)
	{
		...
		if (m_Algorithms[algIdx].CheckAlgPtn(ptnName.c_str()))   // 검사하는 알고리즘이라면
		{
			...
			if (m_Algorithms[algIdx].GetAlgResult())
			{
				stringstream ss;
				ss.str("");
				ss << "Ptn[" << ptnName.c_str() << "00" << GetModelCount() << "] : Alg[" << m_Algorithms[algIdx].GetAlgName() << "] OK.\n";
				CString* logText = new CString(ss.str().c_str());
			...
			else
			{
				stringstream ss;
				ss.str("");
				ss << "Ptn[" << ptnName.c_str() << "00" << GetModelCount() << "] : Alg[" << m_Algorithms[algIdx].GetAlgName() << "] NG.\n";
				CString* logText = new CString(ss.str().c_str());
				PrintLog(logText, DEFECTMSG);
		...
		if (m_nTotalFinAlgCnt == m_nTotalPtnCnt * m_nTotalAlgCnt)		//모든 패턴에 대한 모든 알고리즘 검사가 끝났으면
		{
			...
			SendMessage(m_hWnd, UM_RESMSG, 0, (LPARAM)&m_strResText);

			m_strResText = " ";
			m_nTotalFinAlgCnt = 0;										//(알고리즘 쓰레드) 검사를 완료한 총 알고리즘 개수 증가
			
			//if (GetModelCount() != m_nTotalModelCnt)					//전체 반복 
			//{
			//	m_nFinModelCnt++;
			//	StartInspection();
			//	//SendMessage(m_hWnd, UM_INSPEND, 0, 0);
