...
void CAlgorithmData::RunAlgorithm(HWND hWnd, const cv::Mat& img)
{
	...
	{
		m_bAlgResult = ImageRed(camerahole);
	}

	else if (!strcmp(m_szAlgName, "greenDefect"))
	{
		m_bAlgResult = ImageGreen(camerahole);
	}

	else if (!strcmp(m_szAlgName, "blueDefect"))
	{
		m_bAlgResult = ImageBlue(camerahole);
	}
}
...
bool CAlgorithmData::ImageRed(cv::Point pt_cam)
{
	Point dots[] = {
		Point(260, 280),
		Point(245, 300),
	...
	fillConvexPoly(mask, dots, 8, Scalar(255));
	...
	for (int i = 0; i < m_matImage.rows; i++)
	{
		...
		{
			/*if (mask.at<uchar>(i, j) == 255 && m_matImage.at<Vec3b>(i, j)[2] != 0)
			{
				AfxTrace(_T("%d: %d %d"), static_cast<int>(m_matImage.at<cv::Vec3b>(i, j)[2]), i, j);
			}*/
			//AfxTrace(_T("234234"));
			if (mask.at<uchar>(i, j) == 255 && m_matImage.at<Vec3b>(i, j)[2] < 235)
			{
				AfxTrace(_T("%d: %d %d"), static_cast<int>(m_matImage.at<cv::Vec3b>(i, j)[2]), i, j);
...
bool CAlgorithmData::ImageGreen(cv::Point pt_cam)
{
	Point dots[] = {
	Point(260, 280),
	Point(245, 300),
	...
	for (int i = 0; i < m_matImage.rows; i++) {
		for (int j = 0; j < m_matImage.cols; j++) {
			if (mask.at<uchar>(i, j) == 255 && m_matImage.at<Vec3b>(i, j)[1] < 235)
			{
				AfxTrace(_T("%d: %d %d"), static_cast<int>(m_matImage.at<cv::Vec3b>(i, j)[1]), i, j);
...
bool CAlgorithmData::ImageBlue(cv::Point pt_cam)
{
	Point dots[] = {
	Point(260, 280),
	Point(245, 300),
	...
	for (int i = 0; i < m_matImage.rows; i++) {
		for (int j = 0; j < m_matImage.cols; j++) {
			if (mask.at<uchar>(i, j) == 255 && m_matImage.at<Vec3b>(i, j)[0] < 235)
			{
				AfxTrace(_T("%d: %d %d"), static_cast<int>(m_matImage.at<cv::Vec3b>(i, j)[0]), i, j);
