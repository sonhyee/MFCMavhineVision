...
class CInspectionHandler():
    ...
    # global CamQueue, AlgQueue
    # CamQueue = queue.Queue()
    # AlgQueue = queue.Queue()
    
    # global CamLock
    # CamLock = threading.Lock()
    # global Cam_cv
    # Cam_cv = threading.Condition(CamLock)
    
    # global AlgLock
    # AlgLock = threading.Lock()
        
    # global Alg_cv 
    # Alg_cv = threading.Condition(AlgLock)
    ...
    def ProcessingCamQueue(self):
        while self.m_bRunCamThread:
            with self.m_CamLock:
                self.m_Cam_cv.wait_for(lambda: not self.m_CamQueue.empty() or not self.m_bRunCamThread)

            if not self.m_bRunCamThread:    # Exit if camera thread termination is requested
                print('thread done')
                return

            option = self.m_CamQueue.get() # 원소 반환 및 제거
            #self.m_CamQueue.get()    # Retrieve and store the task if there is something to do, then delete it from the queue
            print(str(list(self.m_CamQueue.queue)))
            
            
            match option:
                case 'CONNECT':
                    #self.signal.emit()
                    self.m_Camera.ConnectCam(self.parent)
                    #logPrint(self.m_Camera.ConnectCam())
                    #self.m_Camera.ConnectCam()    # Connect the camera
                    print('here')
                    break
                case 'SETCAMPARAMS':
                    self.m_Camera.SetCamParams(self.m_Patterns[self.m_nGrabCnt].GetShutterSpeed())    # Set parameters
                    print(self.m_Patterns[self.m_nGrabCnt].GetShutterSpeed())
                    print('here1')
                    break
                case 'GRAB':
                    img_name = self.m_Patterns[self.m_nGrabCnt].GetPtnName() + "00" + str(self.get_model_count()) + ".jpg"
                    print(img_name)
                    print('here2')
                    #img_name = tmp.c_str()
                    ...
                    if img.empty():
                        ...
                        logText = "Inspection Finished.\n"
                        #print_log(log_text)
                        self.parent.signal.emit(logText)
                        
                        self.m_nFinModelCnt = 0

                        #self.send_message(self.hwnd, UM_INSPEND, 0, 0)
                        continue

                    self.m_Camera.Grab(self.m_Patterns[self.m_nGrabCnt].GetPtnName())    # Capture

                    self.m_Patterns[self.m_nGrabCnt].SetPtnImg(img)
                    pattern_ptr = id(self.m_Patterns[self.m_nGrabCnt])
                    self.send_message(self.hwnd, UM_PRINTIMG, pattern_ptr, 0)

                    self.m_AlgLock.acquire()
                    self.m_AlgQueue.append((self.m_Patterns[self.m_nGrabCnt].GetPtnName(), self.m_nGrabCnt))    
                    self.m_Alg_cv.notify_all()    # Wake up all sleeping algorithm threads to enable parallel processing
                    self.m_AlgLock.release()

                    self.m_nGrabCnt += 1    # Increment the number of patterns captured so far
                    break
                case 'DISCONNECT':
                    if self.m_Camera.CheckCamOn():
                        self.m_Camera.DisconnectCam()    # Disconnect the camera after checking if it is connected
                    break
                
    def ProcessingAlgQueue(self):
        while self.m_bRunAlgThread:
            with self.m_AlgLock:
                self.m_Alg_cv.wait_for(lambda: not self.m_AlgQueue.empty() or not self.m_bRunCamThread)

            if not self.m_bRunAlgThread:
                self.m_AlgLock.release()
                return

            # ptn_name = self.m_AlgQueue.front().first
            # ptn_num = self.m_AlgQueue.front().second
            ptn_name, ptn_num = self.m_AlgQueue.queue[0]
            alg_idx = self.m_nPtnChkAlgCnt
            print(ptn_name, ptn_num)
            self.m_PtnChkAlgCntLock.acquire()
            self.m_nPtnChkAlgCnt += 1

            if self.m_nPtnChkAlgCnt == self.m_nTotalAlgCnt:
                self.m_AlgQueue.get()
                self.m_nPtnChkAlgCnt = 0
            self.m_PtnChkAlgCntLock.release()
            self.m_AlgLock.release()

            if self.m_Algorithms[alg_idx].CheckAlgPtn(ptn_name):
                self.m_Algorithms[alg_idx].RunAlgorithm(self.hwnd, self.patterns[ptn_num].GetPtnImg())

                if self.m_Algorithms[alg_idx].GetAlgResult():
                    #ss = ""
                    logText = f"Ptn[{ptn_name}]00{self.get_model_count()} : Alg[{self.m_Algorithms[alg_idx].GetAlgName()}] OK.\n"
                    self.parent.signal.emit(logText)
                    # log_text = CString(ss)
                    # print_log(log_text, DEFECTMSG)
                else:
                    #ss = ""
                    logText = f"Ptn[{ptn_name}]00{self.get_model_count()} : Alg[{self.m_Algorithms[alg_idx].GetAlgName()}] NG.\n"
                    self.parent.signal.emit(logText)
                    # log_text = CString(ss)
                    # print_log(log_text, DEFECTMSG)

                    self.m_strResText += self.m_Algorithms[alg_idx].GetAlgName() + " "

            self.m_TotalFinAlgCntLock.acquire()
            self.m_nTotalFinAlgCnt += 1

            if self.m_nTotalFinAlgCnt == self.m_nTotalPtnCnt * self.m_nTotalAlgCnt:
                #ss = ""
                ...
                #send_message(self.hwnd, UM_RESMSG, 0, LPARAM(self.m_strResText))

                self.m_strResText = " "
                self.m_nTotalAlgCnt = 0
                self.m_nFinModelCnt += 1
                StartInspection()

            self.m_TotalFinAlgCntLock.release()
...
#             with self.parent.m_CamLock:
#                 self.parent.m_Cam_cv.wait_for(lambda: not self.parent.m_CamQueue.empty() or not self.parent.m_bRunCamThread)

#             if not self.parent.m_bRunCamThread:    # Exit if camera thread termination is requested
#                 return
...
#                     break
#                 case 'GRAB':
#                     tmp = self.parent.m_Patterns[self.parent.m_nGrabCnt].GetPtnName() + "00" + str(self.parent.get_model_count()) + ".jpg"
#                     img_name = tmp.c_str()
...
#                     self.parent.m_Camera.Grab(self.parent.m_Patterns[self.parent.m_nGrabCnt].GetPtnName())    # Capture

#                     self.parent.m_Patterns[self.parent.m_nGrabCnt].SetPtnImg(img)
#                     pattern_ptr = id(self.parent.m_Patterns[self.parent.m_nGrabCnt])
#                     #self.Ins.send_message(self.hwnd, UM_PRINTIMG, pattern_ptr, 0)

#                     self.parent.m_AlgLock.acquire()
#                     self.parent.m_AlgQueue.append((self.parent.m_Patterns[self.parent.m_nGrabCnt].GetPtnName(), self.parent.m_nGrabCnt))    
...
#             # ptn_name = self.m_AlgQueue.front().first
#             # ptn_num = self.m_AlgQueue.front().second
#             ptn_name, ptn_num = seself.parentlf.m_AlgQueue.queue[0]
...
#                 self.parent.m_Algorithms[alg_idx].RunAlgorithm(self.parent.hwnd, self.parent.patterns[ptn_num].GetPtnImg())

#                 if self.parent.m_Algorithms[alg_idx].GetAlgResult():
#                     ss = ""
#                     ss += f"Ptn[{ptn_name}]00{self.parent.get_model_count()} : Alg[{self.parent.m_Algorithms[alg_idx].GetAlgName()}] OK.\n"
#                     log_text = CString(ss)
#                     #print_log(log_text, DEFECTMSG)
#                 else:
#                     ss = ""
#                     ss += f"Ptn[{ptn_name}]00{self.parent.get_model_count()} : Alg[{self.parent.m_Algorithms[alg_idx].GetAlgName()}] NG.\n"
#                     log_text = CString(ss)
#                     #print_log(log_text, DEFECTMSG)
